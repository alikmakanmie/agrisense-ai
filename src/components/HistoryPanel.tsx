      ...
      <div className="flex flex-1 flex-col overflow-hidden w-full h-full">
        <div className="flex flex-1 items-center justify-center p-8">
          <div className="text-center max-w-sm">
            ...
            <h3 className="text-2xl font-bold text-foreground mb-3">Belum Ada Riwayat</h3>
            <p className="text-base text-muted/80 leading-relaxed">
              Anda belum melakukan analisis apapun. Kembali ke tab Dashboard untuk memulai analisis lahan pertama Anda.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="flex flex-1 flex-col overflow-hidden w-full h-full">
      ...
      <div className="flex items-center justify-between border-b border-border/50 bg-surface/40 backdrop-blur-md px-8 py-5 shrink-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-foreground">Riwayat Analisis</h2>
            <p className="text-[11px] font-medium text-muted">Akses laporan AI yang telah tersimpan</p>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex-1 relative z-10">
                <div className="flex items-center gap-3 mb-3">
                  <h3 className="text-base font-bold text-foreground group-hover:text-accent transition-colors">Laporan Analisis • {item.formData.luasLahan} ha</h3>
                ...
                <div className="flex flex-wrap gap-2.5 mt-4">
                  <span className="inline-flex items-center gap-1.5 rounded-lg border border-border/50 bg-background/50 px-3 py-1.5 text-[11px] font-medium text-muted shadow-sm">
                    Tanah: <span className="text-foreground font-bold">{item.formData.kondisiTanah}</span>
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-lg border border-border/50 bg-background/50 px-3 py-1.5 text-[11px] font-medium text-muted shadow-sm">
                    Cuaca: <span className="text-foreground font-bold">{item.formData.cuacaDominan}</span>
                  </span>
                  <span className="inline-flex items-center gap-1.5 rounded-lg border border-border/50 bg-background/50 px-3 py-1.5 text-[11px] font-medium text-muted shadow-sm">
                    Modal: <span className="text-foreground font-bold">Rp {parseInt(item.formData.modalAwal).toLocaleString("id-ID")}</span>
                  </span>
                </div>
              </div>
              ...
              <div className="shrink-0 flex items-center gap-5 w-full sm:w-auto relative z-10 border-t sm:border-t-0 sm:border-l border-border/50 pt-4 sm:pt-0 sm:pl-6">
                {item.result.meta && (
                  <div className="hidden sm:block text-right">
                    ...
                    <p className="text-[10px] text-accent mt-1">{item.result.data?.recommendations.length || 0} Rekomendasi</p>
                  </div>
                )}
